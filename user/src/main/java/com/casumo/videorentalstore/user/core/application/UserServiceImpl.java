...
public class UserServiceImpl implements UserService {
	...
	public UserServiceImpl(CommandGateway commandGateway, UserRepository userRepository){
		this.commandGateway = commandGateway;
		this.userRepository = userRepository;
	}
	
	@Override
	public void createUser(UUID userId, String name) {
	...
	public Iterable<User> getAllUsers() {
		return this.userRepository.findAll().stream().map(this::toUserDto).collect(Collectors.toList());
	}
	
	@Override
	public Optional<User> getUserById(UUID id) {
		return this.userRepository.findById(id).map(this::toUserDto);
	}
	
	private User toUserDto (com.casumo.videorentalstore.user.core.persistence.UserEntity user) {
